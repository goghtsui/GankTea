...
public class SwipeCardsView extends LinearLayout {
    ...
    private static final String TAG = "SwipeCardsView";
    ...
    // 定义申请常量列表
    @IntDef({STATUS_DISPLAY, STATUS_CLOSE})
    ...
    public void notifyDatasetChanged(int index) {
        if (canResetView()) {
            Log.d(TAG, " notifyDatasetChanged canResetView=" + index);
            refreshUI(index);
        } else {
            Log.d(TAG, " notifyDatasetChanged can not Reset View=" + index);
            mWaitRefresh = true;
    ...
    private boolean inPicRect(float pointX, float pointY) {
        ImageView meiziPic = (ImageView) findViewById(R.id.meizi_card_item_imageview);
        if (null == meiziPic) {
            Log.d(TAG, "no pic, the point is no in pic rect.");
            return false;
        }
        int sideWidth = (getRight() - meiziPic.getRight()) / 2;
        int sideHeight = (getBottom() - meiziPic.getBottom()) / 2;
        if (pointX > sideWidth && pointX < (getRight() - sideWidth)
                && pointY > sideHeight && pointY < (getBottom() - sideHeight)) {
            Log.d(TAG, "the point is in pic rect.");
            return true;
        }

        Log.d(TAG, "the point is no in pic rect.");
        return false;
    }
    ...
    public boolean dispatchTouchEvent(@NonNull MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                // 判断Y坐标的变化距离及变化速率
                // 如果移动距离大于150，并且绝对速率值大于两倍的最小速率，则视为图片拖动，否则触摸事件交给上层view
                int moveY = (int) ev.getRawY();
                int moveX = (int) ev.getRawX();
                int distanceY = Math.abs(moveY - mInitialMotionY);
                if (Math.abs(velocityY) > mMinVelocity * 2 && distanceY > 150) {
...
//        child.setAlpha(alpha);*/
    ...
    public void autoStart(){
        ...
        for(int i = 0; i < count; i++){
            ...
            float scale = 1 - scaleOffsetStep * (count - 1 - i);
            ...
            switch (mCardStatus) {
                case STATUS_CLOSE:
                    PropertyValuesHolder display = PropertyValuesHolder.ofFloat("rotation",  15 * (count - 1 - i));
                    ObjectAnimator displayAnimator = ObjectAnimator.ofPropertyValuesHolder(child, display);
                    displayAnimator.setInterpolator(sInterpolator);
                    displayAnimator.addListener(new Animator.AnimatorListener() {
                        @Override
                    ...
                    break;
                case STATUS_DISPLAY:
                    PropertyValuesHolder close = PropertyValuesHolder.ofFloat("rotation",  0);
                    ObjectAnimator closeAnimator = ObjectAnimator.ofPropertyValuesHolder(child, close);
     ...
     *
     * @param type {@link com.gogh.fortest.card.view.SwipeCardsView.SlideType}
     */
