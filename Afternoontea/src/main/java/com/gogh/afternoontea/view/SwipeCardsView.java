...
public class SwipeCardsView extends LinearLayout {
    ...
    public void notifyDatasetChanged(int index) {
        if (canResetView()) {
            Logger.d(TAG, " notifyDatasetChanged canResetView=" + index);
            refreshUI(index);
        } else {
            Logger.d(TAG, " notifyDatasetChanged can not Reset View=" + index);
            mWaitRefresh = true;
    ...
    private boolean inPicRect(float pointX, float pointY) {
        ...
        if (null == meiziPic) {
            Logger.d(TAG, "no pic, the point is no in pic rect.");
            return false;
        }
        ...
        if (pointX > sideWidth && pointX < (getRight() - sideWidth)
            ...
            Logger.d(TAG, "the point is in pic rect.");
            return true;
        }

        Logger.d(TAG, "the point is no in pic rect.");
        return false;
    }
