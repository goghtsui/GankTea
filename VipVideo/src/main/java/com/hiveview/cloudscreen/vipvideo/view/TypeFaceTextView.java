...
import com.hiveview.cloudscreen.vipvideo.util.TypefaceUtil.TypefaceFile;
...
public class TypeFaceTextView extends TextView {
            ...
            for (int i = 0; i < typedArray.getIndexCount(); i++) {
                ...
                if (attr == R.styleable.attr_domy_text_usertypeface) {
                    try {
                        ...
                        TypefaceFile file = TypefaceUtil.getTypefaceNameFromAttrValue(typeface);
                        selectTypeface(file);
                    } catch (Exception e) {
                        Logger.e(TAG, "系统缺少字体 e=" + e.toString());
                    }
                }
            }
            typedArray.recycle();
     ...
     * @Description
     */
    private void selectTypeface(TypefaceFile file) {
        try {
            setTypeface(TypefaceUtil.getTypeface(CloudScreenApplication.getInstance().getApplicationContext(), file));
        } catch (Exception e) {
    ...
    public void setTypeface(TypefaceFile typefaceFile) {
        try {
